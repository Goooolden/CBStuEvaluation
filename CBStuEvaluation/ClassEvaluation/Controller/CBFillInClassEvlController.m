...
@interface CBFillInClassEvlController ()<UITableViewDataSource, UITableViewDelegate>
...
@implementation CBFillInClassEvlController

- (void)viewDidLoad {
    [super viewDidLoad];
    [self initBarButton];
    self.sectionArray = [CBFateDataFactory fillElvData];
...
- (void)loadListEvent {
    ...
    [BaseHandler executeRequestWithURL:kApi_ListEventByCourse headers:kToken params:@{@"courseId": self.courseId} success:^(id json) {
        if ([json[@"result"] integerValue] == 1) {
            ...
            TableViewSectionObject *section = self.sectionArray[0];
